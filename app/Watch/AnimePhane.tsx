  ...
  const fetchVideoData = async () => {
    try {
      ...
      if (json.sources && json.sources.length > 0) {
        ...
        const defaultSource = modifiedSources.find(
        ...
        const sourceUrl = defaultSource ? defaultSource.url : modifiedSources[0].url;
        setInitialVideoSource(sourceUrl);
        setVideoSource(sourceUrl);
        setSelectedQuality(defaultSource ? defaultSource.quality : modifiedSources[0].quality);
      }
    } catch (error) {
